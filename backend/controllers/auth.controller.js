...
exports.forgotPassword = async (req, res) => {
  try {
    const { identifier, customPassword } = req.body;
    ...
    if (customPassword && customPassword.trim().length >= 6) {
      ...
      // Auto-generate a letters-only password
      finalPassword = generateLettersOnlyPassword();
      passwordWasAutoGenerated = true;
    }

    /* ── Update account ── */
    account.hashedPassword      = finalPassword;
    account.passwordResetUsedAt = new Date();
    await account.save(); // pre-save hook hashes the password

    /* ── Send email ── */
    if (passwordWasAutoGenerated) {
      // Email the generated password so user knows what it is
      await sendPasswordResetEmail(account.emailAddress, finalPassword);
      return res.json({
        success: true,
        autoGenerated: true,
        message: `A new auto-generated password has been sent to ${account.emailAddress}.`,
      });
    } else {
      // Custom password set — just confirm, don't email the password
      await sendPasswordResetEmail(account.emailAddress, null);
      return res.json({
        success: true,
        autoGenerated: false,
